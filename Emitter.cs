...
namespace CourseWork_V.Dodonova
{
    public class Emitter
    {
        ...
        public float Xl = 300; // X координата положения конца линии
        public float Yl = 300; // Y координата положения конца линии
        ...
        public void UpdateState() // Метод обновления состояния системы
        {
            ...
            ParticlesCount = particles.Count; // Кол-во активных частиц равно кол-во частиц
        ...
        {
            var b = new Pen(Color.White); // создали кисть для рисования
            ...
            foreach (var point in impactPoints)
            {
                point.Render(g); // Рендер специальных точек
            }

            g.DrawLine(b, X, Y, Xl, Yl);
        }
    }
}
